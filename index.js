...
btn1.addEventListener('click',function(){
    ...
    if(!first1.value  || !email1.value || !formFile.value || !ph_no.value){
      ...
      card1.innerHTML += `
  <div class="card" style="max-width: 500px; max-height:300px; margin-bottom:10px; background-color:#0d0d0d;">
  <div class="row g-0">
  
    <div class="col-15">
      <div class="card-body">
        <h5 id="one" class="card-title">${first1.value} </h5>
        <p id="one" class="card-text">${op1.checked ? op1.value : op2.checked ? op2.value : 'Others'}</p>
        <p id="one" class="card-text">${email1.value}</p>
        <p id="one" class="card-text">${formFile.value} </p>
        <p id="one" class="card-text">${ph_no.value}</p>
        <p id="one" class="card-text">${values}</p>
      </div>
    </div>
  </div>
  </div>`
